...
// English grammar helper for "unit"
function units(n) {
...
function showConnectionWarning(show) {
  ...
  if (!el) {
    ...
    el.textContent = 'Connection problem — retrying...';
...
socket.on('connect', () => {
  ...
  if (joined && myName) {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem('pasture_session'));
...
document.getElementById('join-btn').addEventListener('click', () => {
  ...
  if (!code || code.length !== 4) { errorEl.textContent = 'Enter a 4-character room code.'; return; }
  ...
  socket.emit('room:join', { code, name }, (res) => {
    ...
    try { localStorage.setItem('pasture_session', JSON.stringify({ code, name })); } catch(e) {}
    document.getElementById('join-form').classList.add('hidden');
    document.getElementById('waiting-area').classList.remove('hidden');
  });
});
...
function renderHerd(herd, containerId) {
  ...
  container.innerHTML = html || '<span style="color:var(--text-muted)">No animals</span>';
}

function renderHerdValue(herd, elId) {
  const el = document.getElementById(elId);
  if (el) el.textContent = 'Herd value: ' + calcHerdValue(herd);
}
...
function updatePastureInfo() {
  ...
  document.getElementById('pasture-text').textContent = `Pasture: ${total} / ${cap}`;
...
function showPhaseWaiting(msg) {
  ...
  document.getElementById('round-result-round').textContent = msg || 'Waiting...';
...
function showPhaseA() {
  ...
  document.getElementById('budget-display').textContent = `Your limit: ${maxAcq} ${units(maxAcq)}`;
    ...
    row.innerHTML = `
      <div class="acquire-animal">
        ...
        <div class="info">
          ...
          <div class="pool-count">In pool: ${poolCount}</div>
        </div>
      </div>
...
function updateAcquireTotal(maxAcq) {
  ...
  document.getElementById('selected-total').textContent = `Selected: ${total} / ${maxAcq} ${units(maxAcq)}`;
  ...
  for (const type of ANIMAL_TYPES) {
    if (herd[type] > 0) {
      ...
      btn.innerHTML = `
        ...
        <span class="value">value: ${ANIMALS[type].value}</span>
      `;
      btn.addEventListener('click', () => {
...
function showPhaseD() {
  ...
  for (const p of others) {
    ...
    item.innerHTML = `
      ...
      <span class="player-herd-val">value: ${p.herdValue}</span>
    `;
    item.addEventListener('click', () => {
      ...
      document.getElementById('punish-btn').textContent = `Punish: ${p.name}`;
    });
    list.appendChild(item);
  }

  document.getElementById('punish-btn').disabled = true;
  document.getElementById('punish-btn').textContent = 'Punish: ...';
}
...
socket.on('game:state', (s) => {
  ...
  if (s.phase === 'phaseA') showPhaseA();
  else if (s.phase === 'phaseB') showPhaseWaiting('Checking pasture...');
  else if (s.phase === 'waiting_for_instructor') showPhaseWaiting('Waiting for instructor...');
    ...
    document.getElementById('gameover-score').textContent = calcHerdValue(s.herd) + ' pts';
    document.getElementById('gameover-rank').textContent = 'Waiting for results...';
...
socket.on('phaseB:result', (data) => {
  if (data.famine) {
    ...
    lossList.innerHTML = html || '<div class="losses-item">You lost nothing</div>';

    renderHerd(state.herd, 'famine-remaining-herd');
    document.getElementById('famine-remaining-value').textContent = 'Remaining: ' + data.herdValue;
    document.getElementById('famine-pasture').textContent = 'New pasture capacity: ' + data.pastureCapacity;
...
socket.on('phaseD:result', (data) => {
  ...
  if (data.wasTarget && data.punishedBy > 0) {
    ...
    html += `<div class="punishment-result was-punished">
      <p>⚡ You were punished by <strong>${data.punishedBy}</strong> player${data.punishedBy === 1 ? '' : 's'}:</p>
    ...
    html += `<div class="punishment-result safe"><p>✅ Nobody punished you.</p></div>`;
  }

  if (data.punished) {
    html += `<div class="punishment-result gave-punishment">
      <p>🔨 You punished: <strong>${escapeHtml(data.punished)}</strong></p>
      ${data.lost ? `<p>Cost of punishment: ${ANIMALS[data.lost].emoji} ${ANIMALS[data.lost].name}</p>` : ''}
    </div>`;
  } else {
    html += `<div class="punishment-result"><p>You punished nobody.</p></div>`;
  }

  document.getElementById('phaseD-result-content').innerHTML = html;
  renderHerd(data.herd, 'phaseD-result-herd');
  document.getElementById('phaseD-result-value').textContent = 'Herd value: ' + data.herdValue;
});
...
function showRoundResults() {
  ...
  document.getElementById('round-result-value').textContent = 'Herd value: ' + calcHerdValue(state.herd);
  document.getElementById('round-result-pasture').textContent =
    `Pasture capacity: ${state.pastureCapacity}`;
...
socket.on('game:over', (data) => {
  showScreen('gameover-screen');
  document.getElementById('gameover-score').textContent = data.yourScore + ' pts';
  document.getElementById('gameover-rank').textContent = `Place ${data.yourRank} of ${data.scores.length}`;
...
socket.on('game:reset', (data) => {
  ...
  document.getElementById('join-form').classList.add('hidden');
  document.getElementById('waiting-area').classList.remove('hidden');
  document.getElementById('waiting-msg').textContent = `Game 2 — With Communication. Waiting for start...`;
});
...
socket.on('game:paused', () => {
  ...
  if (!overlay) {
    ...
    overlay.innerHTML = '<div style="text-align:center;color:#fff;font-size:1.5rem;font-weight:700;"><div style="font-size:3rem;margin-bottom:0.5rem;">⏸</div>Game paused</div>';
...
function updateTopBar() {
  ...
  if (roundEl) {
    ...
    roundEl.textContent = mr > 0 ? `Round ${state.round || 0} of ${mr}` : `Round ${state.round || 0}`;
  }
